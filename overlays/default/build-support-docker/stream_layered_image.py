...
def archive_paths_to(obj, paths, mtime):
    ...
    with tarfile.open(fileobj=obj, mode="w|") as tar:
        ...
        # FIXME
        tar.addfile(apply_filters(
            nix_root(dir("/scratch_na"))))
        tar.addfile(apply_filters(
            nix_root(dir("/scratch_na/users"))))
        tar.addfile(apply_filters(
            nix_root(dir("/scratch_na/users/bguibertd"))))
        tar.addfile(apply_filters(
            nix_root(dir("/scratch_na/users/bguibertd/nix"))))
        tar.addfile(apply_filters(
            nix_root(dir("/scratch_na/users/bguibertd/nix/store"))))
