{
  ...
  outputs =
    ...
    rec {
      ...
      devShells.default = with pkgs; mkDevShell {
        name = "pkgs";
        mods = [ ];
        shellHook = ''
      ...
      lib.findModDeps = pkgs: with inputs.nixpack.lib; with builtins; let
        ...
        adddeps = s: pkgs: add s
          (filter
            (p: /*builtins.trace "adddeps: ${nixpkgs.lib.generators.toPretty { allowPrettyValues = true; } p}"*/
              p != null
      ...
      overlay = final: prev:
        let
              ...
              stdenvNoCC.mkDerivation ({
                ...
                shellHook = ''
                  ...
                  ${if mods != [] then ''
                    ...
                    ${if autoloads != "" then "echo_cmd ml load ${autoloads}" else ""}
                    echo_cmd ml av
                  '' else ""}
            ...
            packs = {
              bootstrap = {
                ...
                pack = import ./packs/bootstrap.nix {
                  ...
                  extraConf = import ./hosts/${host}/bootstrap.nix { inherit rpmExtern;
                  pkgs = final.pkgs;
                };
              };
            };
              ...
              pack = import ./packs/core.nix inputs.nixpack.lib.packs
                {
                  ...
                  pkgs = final.pkgs;
                  extraConf = (import ./hosts/${ host}/core.nix { inherit rpmExtern inputs;
                  pkgs = final.pkgs;
                }) // {
              repos = [
              (builtins.path { name="hpcw-repo";
          ...
          in overlaySelf
            # blom configurations
            // (inputs.nixpkgs.lib.listToAttrs (map (attr: with attr; {
          ...
          # hpcw configurations
          // (inputs.nixpkgs.lib.listToAttrs (inputs.nixpkgs.lib.flatten (map (attr: with attr;
          [
          {
          ...
          variants = [
            { name = ""; prefs = { }; } # default HPCW
            {
              ...
              devShell = pack: with final.pkgs; mkDevShell {
                name = "hpcw-ifs";
